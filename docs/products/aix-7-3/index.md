...
> IBM Power 上の UNIX OS。AIX 7.3 のカーネル/ファイルシステム/ネットワーク/セキュリティ等を、コマンド/設定値/用語+運用手順 の 5 軸で網羅的に整理した二次資料。
...
| [03. 設定値一覧](02-settings.md) | tunable と /etc/* 設定ファイル | **167 件** |
| [04. 用語集](03-glossary.md) | AIX 固有概念（LPAR / VIOS / SMIT / ODM 等）+ 略号 | **104 件** |
| [05. タスクプレイブック](04-playbook.md) | 習熟度（入門/中級/上級）× シーン マトリクス | 22 セル |
...
| **[09. 設定手順](08-config-procedures.md)** | 代表的な設定変更シナリオ（コマンド+期待出力+検証+ロールバック） | **12 手順** |
| **[10. 障害対応手順](09-incident-procedures.md)** | 典型障害の切り分けフロー（症状→確認→仮説→対処→検証） | **15 手順** |

各章の用語・コマンド・設定値エントリには、該当する **08-設定手順** / **09-障害対応手順** への逆リンクが付与されています。
...
v2 の 8 セクション構成（概要/コマンド/設定値/用語/プレイブック/トラブル/マニュアルマップ/出典）に **設定手順** と **障害対応手順** の 2 章を追加し、用語・コマンド・設定値の各エントリから関連手順への逆リンクを張ることで、

- **コマンド名で引く** → 「このコマンドを使う具体的な手順」へジャンプ
- **設定値で引く** → 「この設定を変える具体的な手順」へジャンプ
- **用語で引く** → 「この概念が登場する具体的な手順」へジャンプ

の 3 通りのアクセスパスを実現しました。
