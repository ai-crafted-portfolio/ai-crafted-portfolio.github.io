...
AIX 管理者が日常的に必要な設定変更を、**目的 → 前提 → 手順 → 期待出力 → 検証 → ロールバック → 関連** の 7 部構成で記述。**vi 編集は曖昧表現を避け、完全な example block** で示す（ユーザ preference 「VBA ボタン作成のように、ユーザが実施しなければならない作業はもれなく記載」と整合）。
